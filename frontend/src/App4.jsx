    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
        <Scheduler
            ...
            onConfirm={async (event, action) => {
                ...
                if (action === "create") {
                    // debugger
                    // Generar un nuevo event_id (puedo usar un contador o timestamp)
                    ...
                    // Validar que el ID no exista aún (extremadamente raro, pero seguro)
                    while (events.some(e => e.event_id === newEventId)) {
                        newEventId = Date.now() + Math.floor(Math.random() * 1000); // añadir aleatorio extra por si acaso
                    }
                    // const newEvent = {
                    //     ...event,
                    //     event_id: newEventId
                    ...
                    const newEvent = {
                        ...
                        // Agrega defaults si lo deseas:
                        title: event.title || "Nuevo Evento",
                        subtitle: event.subtitle || "",
                        descripcion: event.descripcion || "",
                        user_id: event.user_id || null, // 1 o null si quieres forzar selección
                        color: "blue"
                    };
                    ...
                    setEvents(prev => {
                        ...
                        console.log("Eventos actualizados (dentro de setState):", updated);
                        return updated;
                    });
